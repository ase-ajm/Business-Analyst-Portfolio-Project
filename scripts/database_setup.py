...
cursor.execute("""
CREATE TABLE IF NOT EXISTS bitesandco (
...
with open(os.path.join(DATA_DIR, "bitesandco_user_behavior.csv"), newline="") as f:
    for row in csv.DictReader(f):
        cursor.execute("INSERT OR IGNORE INTO bitesandco VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
